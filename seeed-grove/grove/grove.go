...
// OverSample averages several readings from the ADC.
// Ensure num is not zero if you don't like unexpected panics.
func (a ADC) OverSample(num uint16) uint16 {
	var sum uint32
	for range num {
		sum += uint32(a.ReadAnalogValue())
	}

	rounded := (sum + uint32(num)/2) / uint32(num)
